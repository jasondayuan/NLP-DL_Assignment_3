    ...
    def forward(
        ...
        **kwargs,
        ...
        if not cached:
            ...
            query, key, value = self.c_attn(hidden_states[:, -num_tokens:, :]).split(self.split_size, dim=2) # (B, 1, C)
    ...
    def forward(
        ...
        **kwargs
        ...
        if not cached:
            ...
            output_shape = (-1,) + (num_tokens,) + (hidden_states.size(-1),) # (-1, #tokens, hidden_size)

        # Iterate over all GPT2 layer, i.e. `block`
        for i, block in enumerate(self.h):
            outputs = block(
                hidden_states if not cached else hidden_states[:, -num_tokens:, :],
                attention_mask=attention_mask,
                cached=cached,
                num_tokens=num_tokens
            )
...
class CustomizedGPT2LMHeadModel(GPT2LMHeadModel):
    ...
    def prefix_cache(self, input_ids, attention_mask):
        if self.prev_input_ids == None:
            self.prev_input_ids = input_ids
            self.prev_attention_mask = attention_mask
            return input_ids.size(1), False
        
        prev_seq_len = self.prev_input_ids.size(1)
        new_seq_len = input_ids.size(1)

        cached = True
        prefix_len = min(prev_seq_len, new_seq_len)
        for i in range(min(prev_seq_len, new_seq_len)):
            if not torch.equal(input_ids[:, i], self.prev_input_ids[:, i]):
                prefix_len = i
                break
        ...
        if prefix_len == 0:
            self.clear_cache()
            cached = False
        elif prefix_len < prev_seq_len:
            for block in self.transformer.h:
                block.attn.cache_key = block.attn.cache_key[:, :prefix_len, :]
                block.attn.cache_value = block.attn.cache_value[:, :prefix_len, :]
        self.prev_input_ids = input_ids
        self.prev_attention_mask = attention_mask

        return new_seq_len - prefix_len, cached
    ...
    def forward(
        ...
        num_tokens, cached = self.prefix_cache(input_ids, attention_mask)
